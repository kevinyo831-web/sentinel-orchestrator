...
// ─── Cyberpunk color palette (matches Sentinel Orchestrator) ────
const C = {
...
const MONTHLY_TARGET = Math.ceil(MONTHLY_LOAN * 1.3); // 33,484
const MAX_CAPITAL    = 130_451;                        // 淨值 60%
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      <span style={{ width: 7, height: 7, borderRadius: '50%', background: col,
        boxShadow: `0 0 5px ${col}`, flexShrink: 0 }} />
      <span style={{ fontSize: 10, fontWeight: 700, color: col,
        fontFamily: "'Share Tech Mono',monospace" }}>{rec || '---'}</span>
    </span>
  );
}
...
// ─── Stock row ───────────────────────────────────────────────────
const COLS = '76px 1fr 80px 68px 110px 88px';
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color: C.white,
        fontFamily: "'Share Tech Mono',monospace" }}>{quote?.symbol ?? '...'}</span>
      ...
      <span style={{ fontSize: 11, color: col, textAlign: 'right',
        fontFamily: "'Share Tech Mono',monospace" }}>
...
// ─── Trade modal ─────────────────────────────────────────────────
const iSt = {
  background: '#10111A', border: '1px solid #1A1C28', borderRadius: 3,
  padding: '7px 10px', color: '#E8EAF0', fontSize: 12, outline: 'none',
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        border: `1px solid ${color}40`, borderRadius: 8, padding: 24,
        boxShadow: isBuy ? C.glow : C.warnGlow,
      }}>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <div style={{ padding: '10px 12px', background: C.surface, borderRadius: 4,
            ...
            {total > MAX_CAPITAL && (
              <div style={{ marginTop: 6, fontSize: 10, color: C.warn2,
                fontFamily: "'Share Tech Mono',monospace" }}>
        ...
        <button
          disabled={busy || !lots || !price}
          onClick={() => onConfirm({ symbol: quote.symbol, side, lots: parseInt(lots),
            price: parseFloat(price), type })}
...
function AIPanel({ symbols }) {
  ...
  const run = useCallback(async () => {
    ...
    setAnalysis('');
    try {
      const res  = await fetch('/api/fubon/signals', {
        method: 'POST',
      ...
      setAnalysis(data.aiAnalysis ||
        data.signals?.map(s => `${s.symbol}: ${s.recommendation} (score ${s.score})`).join('\n'));
    } catch (e) {
    ...
    <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 6,
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
        <input value={query} onChange={e => setQuery(e.target.value)}
          style={{ ...iSt, flex: 1, width: undefined, boxSizing: 'border-box' }}
          placeholder="輸入分析問題…" />
...
export default function TradingPage() {
  ...
  const [modal,      setModal]       = useState(null);  // { quote, side }
      ...
      <div style={{ minHeight: '100vh', background: C.bg, color: C.text,
        ...
        <style>{`
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          @media (max-width:768px) { .desktop-only { display:none!important; } .grid-2 { grid-template-columns:1fr!important; } }
        `}</style>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              onClick={() => { setAutoMode(v => !v); notify(autoMode ? '自動交易已暫停' : '自動交易已啟動 (模擬)'); }}
              style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 16px',
          ...
          <div style={{ marginBottom: 14 }}>
            <LoanTracker profit={monthProfit} />
          </div>
          ...
          {/* MAIN GRID */}
          <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16 }}>
            ...
            <div>
              <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 6,
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  ...
                  <button onClick={() => { fetchQuotes(); fetchSignals(); }}
                    style={{ background: 'none', border: 'none', color: C.accent2,
            ...
            {/* RIGHT ── Loan detail + Strategies + Risk */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ marginTop: 12, display: 'grid', gap: 6 }}>
                  {[
                    ...
                    <div key={s.name} style={{
                      ...
                      <Tag label={s.risk}
                        color={s.risk === 'LOW' ? C.accent : s.risk === 'MED' ? C.warn2 : C.warn} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Risk notice */}
              <div style={{ background: `${C.warn}04`, border: `1px solid ${C.warn}20`,
                ...
                <div style={{ marginTop: 10, fontSize: 11, color: C.text2, lineHeight: 1.8,
                  ...
                  <p>• 單筆最大風險：<b style={{ color: C.warn2 }}>2%</b>（$2,609）</p>
                  ...
                  <p style={{ marginTop: 8, fontSize: 10, color: C.text3,
                    fontFamily: "'Share Tech Mono',monospace" }}>
                    ⚠ 交易有風險，本系統不保證獲利。貸款壓力大時應優先控制虧損。
                  </p>
                </div>
              </div>

              {/* Month P&L input (manual) */}
              <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 6, padding: 16 }}>

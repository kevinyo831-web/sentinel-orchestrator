...
export default async function handler(req, res) {
  ...
  // Run technical analysis for every symbol in parallel
  const results = await Promise.allSettled(
    symbols.map(async sym => {
      const hist   = await getHistoricalData(sym, 3);
      const closes = hist.map(h => h.close);
      if (closes.length < 26) throw new Error(`Not enough data for ${sym}`);
      const tech = compositeScore(closes);
      return { symbol: sym, lastClose: closes.at(-1), histLen: closes.length, ...tech };
    })
  );
  ...
  if (aiQuery && geminiKey) {
    try {
      const prompt =
        `你是台股量化交易分析師。以下是技術指標訊號 JSON：\n` +
        JSON.stringify(signals, null, 2) +
        `\n\n問題：${aiQuery}\n\n請給出具體的操作建議，包含進出場價位，150字以內。`;
